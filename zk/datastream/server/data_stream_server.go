...
func createBlockWithBatchCheckStreamEntriesProto(
	...
	if isBatchStart {
		...
		if !shouldSkipBatchEndEntry {
			localExitRoot, err := utils.GetLocalExitRootFromSCStorage(block.NumberU64(), reader, tx)
			if err != nil {
				return nil, err
			}
...
func BuildWholeBatchStreamEntriesProto(
	...
	// the genesis we insert fully, so we would have to skip closing it
	localExitRoot, err := utils.GetBatchLocalExitRoot(lastBlock.NumberU64(), reader, tx)
	if err != nil {
		return nil, err
	}
